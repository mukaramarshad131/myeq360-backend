      ...
      const multerConfig = multer({
        ...
        fileFilter: (req, file, cb) => {
          if (!file.originalname.match(allowedFileTypes)) {
            return cb(new BadRequestException(`Invalid file type! Allowed types: ${allowedFileTypes}`));
          }
          cb(null, true);
        },
      ...
      multerConfig(request, request.res, (err: any) => {
        if (err instanceof multer.MulterError) {
          if (err.code === 'LIMIT_FILE_SIZE') {
            return reject(new BadRequestException(`File size is too large! Maximum size allowed is ${maxFileSize / (1024 * 1024)} MB.`));
          }
